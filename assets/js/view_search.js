...
const formatSongSearchResults = data => {
    ...
    if (songs.length > 0) {
        $("#song-results").html("");
        const template = require("../templates/search/song-card.hbs");
        songs.forEach(song => {
...
const formatMovieSearchResults = data => {
    if (data.total_results > 0) {
        ...
        const template = require("../templates/search/movie-card.hbs");
...
const formatBookSearchResults = data => {
    ...
    $("#book-results").html("");
    const template = require("../templates/search/book-card.hbs");
...
const formatTVSearchResults = data => {
    if (data.total_results > 0) {
        ...
        const template = require("../templates/search/tv-card.hbs");
        shows.forEach(show => {
